...
export default function UserWidget({ setIsOpen }) {
  ...
  const [leadForm, setLeadForm] = useState({
    name: '', phone: '', email: '',
    status: 'Student', urgency: 'Exploring', program: 'UI/UX Portfolio Building Program'
  });

  const messagesEndRef = useRef(null);
  const scrollToBottom = () => messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' });

  useEffect(() => { scrollToBottom(); }, [messages, isLoading, showForm, formSubmitted]);
  ...
  const initSession = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/chat/start', {
        ...
        body: JSON.stringify({ visitor_id: getVisitorId(), source_page: window.location.href })
      });
      if (response.ok) {
      ...
      setMessages([{ id: 'error', role: 'ai', text: 'Hello! I am CarePilot AI. I am having trouble connecting to my backend right now, but feel free to check our website or message us at +91 7994721792!' }]);
    }
  };
  ...
  const handleRestartChat = () => {
    if (confirm("Would you like to clear history and restart your career guide conversation?")) {
      setSessionId(''); setCurrentScore(0); setLeadCategory('Cold lead');
      setShowForm(false); setFormSubmitted(false); setMessages([]);
      setSuggestedReplies(defaultReplies); initSession();
    }
  };
  ...
  const submitQuery = async (queryText) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/chat/message', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ session_id: sessionId, message: queryText, current_score: currentScore, current_page_url: window.location.href })
      });
      if (response.ok) {
        const data = await response.json();
        setCurrentScore(data.new_score); setLeadCategory(data.lead_category);
        setMessages(prev => [...prev, { id: Date.now().toString(), role: 'ai', text: data.reply }]);
        if (data.trigger_form && !formSubmitted) { setShowForm(true); }
        if (data.suggested_replies && data.suggested_replies.length > 0) {
          setSuggestedReplies(data.suggested_replies.map(reply => ({ text: reply, query: reply })));
        } else {
          setSuggestedReplies([]);
        }
      }
    } catch (e) {
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'ai', text: 'Sorry, I lost my connection.' }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleLeadSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/leads/create', {
        ...
        body: JSON.stringify({ name: leadForm.name, phone: leadForm.phone, email: leadForm.email, current_status: leadForm.status, urgency: leadForm.urgency, interested_program: leadForm.program, lead_score: currentScore, source_page: window.location.href, conversation_summary: `Student ${leadForm.name} looking into ${leadForm.program}.` })
      });
      if (response.ok) {
        setFormSubmitted(true); setShowForm(false);
        setMessages(prev => [...prev, { id: Date.now().toString(), role: 'ai', text: `Thank you, ${leadForm.name}! Your career profile is saved. I've notified our admissions counselor. Would you like to connect directly on WhatsApp now to get immediate batch schedules and brochures?` }]);
        const leadData = await response.json();
        fetch('http://localhost:8000/api/leads/send-whatsapp-notification', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lead_id: leadData.id }) });
      }
    } catch (e) { alert("Failed to submit profile."); }
  };

  const getWhatsAppLink = () => `https://wa.me/917994721792?text=${encodeURIComponent(`Hi Portfolio Builders! My name is ${leadForm.name || 'Student'}. I just completed the CarePilot AI career assessment and I am interested in details regarding the ${leadForm.program || 'programs'}. Please connect me with a counselor.`)}`;
    ...
    <>
      <div className="carepilot-header">
        <div className="carepilot-header-title">
          ...
          <div><h4>CarePilot AI</h4><span className="subtitle">Your 24/7 Career Guide</span></div>
        </div>
        <div className="carepilot-header-actions">
          {sessionId && <button className="carepilot-action-icon" onClick={handleRestartChat} title="Reset Chat">🔄</button>}
      ...
      <div className="carepilot-messages">
        {messages.map(msg => <div key={msg.id} className={`carepilot-message ${msg.role}`}>{msg.text}</div>)}
        ...
        {showForm && !formSubmitted && (
          <div className="carepilot-lead-form-card">
            ...
            <form onSubmit={handleLeadSubmit}>
              <input type="text" placeholder="Your Full Name" value={leadForm.name} onChange={e => setLeadForm({...leadForm, name: e.target.value})} required />
              ...
              <input type="tel" placeholder="WhatsApp Phone Number" value={leadForm.phone} onChange={e => setLeadForm({...leadForm, phone: e.target.value})} required />
        ...
        {formSubmitted && (
          <div className="carepilot-whatsapp-cta-card">
            ...
            <a href={getWhatsAppLink()} target="_blank" rel="noopener noreferrer" className="whatsapp-cta-button">Start WhatsApp Chat</a>
          </div>
        )}
        
        {isLoading && <div className="carepilot-message ai"><div className="carepilot-loading-dots"><span></span><span></span><span></span></div></div>}
        <div ref={messagesEndRef} />
      </div>

      {!showForm && !formSubmitted && suggestedReplies.length > 0 && !isLoading && (
        <div className="carepilot-quick-replies">
          {suggestedReplies.map((reply, idx) => <button key={idx} onClick={() => submitQuery(reply.query)}>{reply.text}</button>)}
        </div>
      )}

      <div className="carepilot-input-area">
        <form className="carepilot-input-form" onSubmit={handleSubmit}>
          <input type="text" className="carepilot-input" placeholder="Ask CarePilot career questions..." value={inputValue} onChange={(e) => setInputValue(e.target.value)} disabled={isLoading || showForm} />
